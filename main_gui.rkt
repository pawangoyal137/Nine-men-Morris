...
(define (f2m w x y me)
  ...
  (define black-pressed (findf (lambda (t) (send t over-it? x y)) (filter (lambda (t) (is-a? t gamepieceu%)) pieces))) ;if we click on a balck this is that #f otherwise
 ...
 (not (equal? #f (index-of (neighbours (get-field s nl) (get-field r nl) (get-field c nl)) (list  (get-field s tbm) (get-field r tbm) (get-field c tbm)))))
                ) (begin (print-board w)
                       (move-piece w (get-field s tbm) (get-field r tbm) (get-field c tbm) (get-field s nl) (get-field r nl) (get-field c nl))
                         (if (closed-mill? w (get-field s nl) (get-field r nl) (get-field c nl)) (set! state '2r)
                             (let* ((w1 w)
                               ...
                               (begin (set! tbr (removed? w1 w2)) (set! w w2)
                               (if (equal? tbr #f)
                                   (set! state '2p)
...
(define (last-scene w)
  (if (equal? endstate 'w)  (place-image (bitmap/file "you-win.jpg") (/ a 2) (/ a 2) bg)
...
(define (k) (big-bang b (to-draw drawer) (on-mouse mhandler ) (on-tick tick) (stop-when endgame? last-scene)))
